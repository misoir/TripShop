<?php
    header('Content-Type: text/html; charset=utf-8');
    
    define("DIR_INDEX_PHP", DIR_MAIN."index.php");
    
    define("DIR_USERS", DIR_MAIN."users/");
    
    define("DIR_MENUBAR", DIR_MAIN."menu/");
    define("DIR_NEWS_PHP", DIR_MAIN."menu/news.php");
    
    define("DIR_DB_CONNECTION_PHP", DIR_MAIN."tools/db_connection.php");
    define("DIR_HTTP_PHP", DIR_MAIN."tools/http.php");
    
    define("DIR_ARTICLES", DIR_MAIN."articles/");
    define("DIR_READ_PHP", DIR_MAIN."articles/read.php");
    define("DIR_VIEW_PHP", DIR_MAIN."articles/view.php");
    define("DIR_ADDARTICLE_PHP", DIR_MAIN."articles/addArticle.php");
    define("DIR_TRANSACT_ARTICLE_PHP", DIR_MAIN."articles/transact_article.php");
//    echo DIR_MENUBAR;
?>
